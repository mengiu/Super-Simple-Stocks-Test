...
public class SimpleStockServiceImpl implements SimpleStockService {
	...
	public boolean recordTrade(Trade trade) throws Exception{
		...
		try{
			...
			TradeDAO tradeDAO = SpringService.INSTANCE.getBean("tradeDAO", TradeDAO.class);
	...
	private double calculateStockPriceinRange(String stockSymbol, int minutesRange) throws Exception{
		...
		Collection<Trade> trades = CollectionUtils.select(tradeDAO.listTrade(), new StockPredicate(stockSymbol, minutesRange));
	...
	public double calculateGBCEAllShareIndex() throws Exception{
		...
		{
			for (Stock item : stockDAO.listStock())
				stocks.put(item.getStockSymbol(), item);
		}
	...
	public int getTradesNumber() throws Exception {
		TradeDAO tradeDAO = SpringService.INSTANCE.getBean("tradeDAO", TradeDAO.class);
		return tradeDAO.listTrade().size();
	}
